...
function TabApparaten({ devices, powerMap, onDeviceAdded, onDeviceEdited, onDeviceDeleted }) {
  ...
  const openEdit = (d) => { setEditId(d.id); setEditName(d.name); setEditIp(d.ip); setEditPort(d.port); setEditError(null); };
            ...
            editId === d.id ? (
              ...
              <div key={d.id} className="settings-device-row">
                <div className="settings-device-info">
                  ...
                  <div className="settings-device-ip">{d.ip}:{d.port}</div>
                </div>
